...
/*   Updated: 2023/01/05 14:37:40 by frgojard         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (vars->map[++y])
	{
		...
		while (vars->map[y][++x])
		{
			if (vars->map[y][x] == '1')
				...
				mlx_put_image_to_window(vars->mlx, vars->win,
					vars->collectible, x * 32, y * 32);
			else if (vars->map[y][x] == 'P')
				mlx_put_image_to_window(vars->mlx, vars->win,
					vars->player, x * 32, y * 32);
		}
	}
}
...
void	img_init(t_vars *vars)
{
	...
	vars->mlx = mlx_init();
	vars->win = mlx_new_window(vars->mlx, vars->len_x, vars->len_y, "so_long");
